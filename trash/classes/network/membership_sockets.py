...
class DBRB():
  ...
  def __init__(self, Node, ip):
    ...
    self.ip = ip
    ##### Compatibility
    self.visible = [] #this is here for compatibility
    self.average = 0
    self.messages = []
    self.visibility_lost = [] #our visibble neighbours
    #self.packets = 0
  ...
  def start(self):
    self.scheduler.start()
    self.discover_udp_interface = network_sockets.UdpInterface(self._discover_handler, debug=False, port=self.discover_port, interface='')
    self.discover_udp_interface.start()
    self.membership_tcp_interface = network_sockets.TcpInterface(self._membership_handler, debug=False, port=self.discover_port, interface='')
  ...
  def _initiate(self):
    for i in range(0,self.membership_len):
      self.membership.append(self.network + str(i+1))
    self.start = 'RUN'

  def _discover(self):
    #onion discover packet
    discover_packet = pickle.dumps([0, self.Node.tag])
    msg_id = zlib.crc32(str(int(time.time()*1000)).encode())
    signature = self._sign(discover_packet)
    buffer = pickle.dumps([signature, discover_packet])
    self.discover_udp_interface.send(self.bcast_group, buffer, msg_id)
  ...
  def _discover_handler(self, payload, sender_ip):
    ...
    #third layer
    real_payload = pickle.loads(message)
    pdu = real_payload[0]
    if pdu == 0: # Got a discover package
      sender_tag = real_payload[1]
      if (sender_tag != self.Node.tag): #not me
        if len(self.view) > 0: #list no empty, check if already there
          found = False
          for element in range(len(self.view)):
            if sender_ip == self.view[element][0]: #if there...
              self.view[element][2] = self.Node.simulation_seconds # refresh timestamp
              found = True
              break
          if not found:
            self.view.append([sender_ip, sender_tag, self.Node.simulation_seconds, 0])
        else: #Empty neighbours list, just add 
          self.view.append([sender_ip, sender_tag, self.Node.simulation_seconds, 0])
    else:
      return
      
  def _membership_handler(self, payload, sender_ip):
    'When a message of type gossip is received from neighbours this method unpacks and handles it'
    #first layer
    payload = pickle.loads(payload)
    msg_id = payload[0]
  ...
  def _prompt(self, command):
    if (len(command))>=2:
      if command[1] == 'help':
        ...
        self.printview()
      elif command[1] == 'membership':
        self.printmembership()
      elif command[1] == 'sign':
        try:
