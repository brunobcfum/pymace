...
def createNodes(number_of_nodes, application, protocol):
    ...
    process = []
    for i in range(0,number_of_nodes):
        command = "xterm -xrm 'XTerm.vt100.allowTitleOps: false' -T drone" + str(i) + " -hold -e ./main.py drone"+ str(i) + " " + application + " " + str(time_scale) + " " + str(time_limit) + " random_walk " + protocol  + " ipv4 -b 100 -r node &"
        node = subprocess.Popen([
                                    "ip",
                                    "netns",
                                    "exec",
